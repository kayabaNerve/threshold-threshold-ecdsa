...
mod tests {
  ...
  use ciphersuite::{group::{ff::{Field, PrimeField}, Group}, Ciphersuite, Secp256k1};

  fn verify(public_key: <Secp256k1 as Ciphersuite>::G, message_hash: <Secp256k1 as Ciphersuite>::F, r: <Secp256k1 as Ciphersuite>::F, s: <Secp256k1 as Ciphersuite>::F) {
  ...
  fn non_distributed() {
    ...
    let r = <Secp256k1 as Ciphersuite>::F::from_repr((Secp256k1::generator() * x).to_affine().x()).unwrap();
